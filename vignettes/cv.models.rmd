...
author: "Michio Oguro email: paste following code into R: rawToChar(as.raw(c(109, 111, 103, 64, 102, 102, 112, 114, 105, 46, 97, 102, 102, 114, 99, 46, 103, 111, 46, 106, 112)))"
date: "`r Sys.Date()`"
output:
    html_document:
        fig_width: 6
...
`cv.models` funciton evaluate predictive ability of a model.
...
## Controling calculation of metrics
...
Currently, class stratification is implimentd only for classification models.

## Controling method for optimal threshold determination
...
By default, `cv.models` still use Youden's J for determination of the threshold, but users can specify a `list` having optins passed to `optimal.cutpoints` for the `cutpoint.options` argument to control method used for threshold determination.
...
For example, by specifying `list(methods = c("MaxSe", "MaxSp"))` for `cutpoint.options`, `cv.models` can calculate two sets of metrics with the thresholds determined by sensitivity and specificty, yet they bring same results... :).
...
Because cross validation and some modeling methods use stochastic processes, `cv.modles` returns slighly different results everytime.
...
# If seed is set, same result is returned everytime.
cv <- cv.models(
    gbm(Petal.Length ~ ., data = iris, distribution = "gaussian", n.cores = 1),
...
In the following example, `cv.models` calculates performance measures with cross validation for all combination of hyper-parameteres (in this example, `1` and `5` for `interaction.depth` and `1` and `10` for `n.minobsinnode`) of `gbm`
...
After grid search, you can extarct model(s) with best predictive ability using `find.best.models` function for the result of `cv.models`.
In the following example, `find.best.models` fuction extracts model(s) with the highest value of Q^2.
...
* **Always AUC become more than 0.5**
    ...
    <s>When the model is worse than random prediction, i.e., if it produce reveresed prediction, pROC::coords switches positive and negative class when calculating AUC.</s>  
...
* 2018-09-14: 0.1.2
  ...
  * Impliment Class Stratification.
* 2018-01-31: 0.1.1
...
* 2018-01-28: 0.1.0
  ...
  * Implimented Cohen's Kappa for classification model.
